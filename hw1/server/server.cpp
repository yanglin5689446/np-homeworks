...
int processInput(const string& input, vector<string>& tokens){
    ...
    else if(tokens[0] == "check" && tokens.size() == 2)return CHECK;
    return ERROR;
}
    ...
    while(getline(ss, input)){
        ...
        switch(status){
            ...
            case TELL:
                ...
                break;
            case CHECK:
                name_fd_map[tokens[1]] == 0 ? server.send(client, "[Server] This name haven't been used.\n") 
                                      : server.send(client, "[Server] This name have been used by others.\n");
                break;
            case ERROR:
